...
export async function getPaginatedBlogs(
  ...
  const results = await client.fetch(
    `*[_type == "blog" && protected.isHidden != true] | order(date ${date}) {${blogFields}}[${offset}...${offset + 4
    }]`
  );
  return results;
}
...
export async function getPaginatedBlogsByTag(
  ...
  const results = await client.fetch(
    `*[_type == "blog" && tags->name == "${tag}"] | order(date ${date}) {${blogFields}}[${offset}...${offset + 4
    }]`
  );
  return results;
}
...
export async function addSubscriber(subscriber) {
  const results = await sanity.create(subscriber)
  return results;
}
